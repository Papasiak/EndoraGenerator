...
"""
#--------------------------------------------------------------
# Add this temporary fix to train.py (line 1)
import os
os.chdir("C:\\Users\\pansi\\Endora")  # Force working directory
os.environ['TORCH_USE_LIBUV'] = '0'
os.environ['GLOO_SOCKET_IFNAME'] = 'lo'  # Force loopback interface
os.environ['PL_TORCH_DISTRIBUTED_BACKEND'] = 'gloo'  # Override backend
#--------------------------------------------------------------
import torch
...
def main(args, port, pretrained_weights, mode, prr_weight):
    ...
    # Setup data:
    dataset = get_dataset(args) #class 'datasets.kva_image_datasets.Kvasir_CapsuleImages', with length 158933
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            attentions = [item[:, 1:, :] for item in attentions]        # list length 4, each item (120, 257, 384) (pithanon 120,256,384 me entolh attentions[0].shape xrhsh Pdb)
            ...
            with torch.no_grad():
                ...
                b, _, _, _, _ = x.shape     # (5, 24, 3, 128, 128)
                x = rearrange(x, 'b f c h w -> (b f) c h w').contiguous()#torch.Tensor with length 120, each item torch.Size([120, 3, 128, 128])
                x = vae.encode(x).latent_dist.sample().mul_(0.18215)#torch.Tensor with length 120, each item torch.Size([120, 4, 16, 16])
                x = rearrange(x, '(b f) c h w -> b f c h w', b=b).contiguous() #torch.Tensor with length 5, item torch.Size([5, 24, 4, 16, 16]), tensor([148, 244, 606, 917, 860], device='cuda:0')
            ...
            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device) #class 'torch.Tensor', torch.Size([5]), length 5, tensor([148, 244, 606, 917, 860], device='cuda:0')
